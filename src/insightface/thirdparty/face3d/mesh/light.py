...
def get_normal(vertices, triangles):
    ...
    mesh_core_cython.get_normal_core(
        normal,
        tri_normal.astype(np.float32).copy(),
        triangles.copy(),
        triangles.shape[0],
    )
...
def add_light(vertices, triangles, colors, light_positions=0, light_intensities=0):
    ...
    direction_to_lights_n = np.sqrt(
        np.sum(direction_to_lights**2, axis=2)
    )  # [nlight, nver]
    direction_to_lights = direction_to_lights / direction_to_lights_n[:, :, np.newaxis]
    normals_dot_lights = (
        normals[np.newaxis, :, :] * direction_to_lights
    )  # [nlight, nver, 3]
...
def fit_light(image, vertices, colors, triangles, vis_ind, lamb=10, max_iter=3):
    ...
    for k in range(c):
        Y[k * n_vis_ind : (k + 1) * n_vis_ind, :] = image_pixel[k, vis_ind][
            :, np.newaxis
        ]
        A[k * n_vis_ind : (k + 1) * n_vis_ind, :] = (
            texture[k, vis_ind][:, np.newaxis] * harmonic[vis_ind, :]
        )
