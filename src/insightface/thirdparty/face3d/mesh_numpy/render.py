...
def render_colors_ras(vertices, triangles, colors, h, w, c=3):
    ...
    depth_buffer, triangle_buffer, barycentric_weight = rasterize_triangles(
    ...
    ]  # [h*w(tri id in pixel), 3(ver in tri), c(color in ver)]
        ...
        for u in range(umin, umax + 1):
            for v in range(vmin, vmax + 1):
                ...
                if point_depth > depth_buffer[v, u]:
                    ...
                    # tex coord
                    tex_xy = (
                        w0 * tex_coords[tex_tri[0], :]
                        + w1 * tex_coords[tex_tri[1], :]
                        + w2 * tex_coords[tex_tri[2], :]
                    )
                    ...
                    # bilinear
                        ...
                        # next 4 pixels
                        ul = texture[
                            int(np.floor(tex_xy[1])), int(np.floor(tex_xy[0])), :
                        ]
                        ur = texture[
                            int(np.floor(tex_xy[1])), int(np.ceil(tex_xy[0])), :
                        ]
                        dl = texture[
                            int(np.ceil(tex_xy[1])), int(np.floor(tex_xy[0])), :
                        ]
                        dr = texture[
                            int(np.ceil(tex_xy[1])), int(np.ceil(tex_xy[0])), :
                        ]
