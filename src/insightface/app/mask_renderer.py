...
class MaskRenderer:
    def __init__(self, name=DEFAULT_MP_NAME, root="~/.insightface", insfa=None):
        ...
        assert osp.exists(bfm_uv_file), (
            "should contains BFM_UV.mat in your model directory"
        )
    ...
    def params_to_vertices(self, params, H, W):
        ...
        transformed_vertices = self.bfm.transform(
            fitted_vertices, fitted_s, fitted_angles, fitted_t
        )
    ...
    def build_params(self, face_image):
        ...
        if self.insfa is None:
            ...
            self.insfa.prepare(
                ctx_id=self.pre_ctx_id,
                det_thresh=self.pre_det_thresh,
                det_size=self.pre_det_size,
            )
    ...
    def render_mask(
        ...
        output = (
            1
            - face3d.mesh.render.render_texture(
                image_vertices,
...
class MaskAugmentation(ImageOnlyTransform):
    ...
    def apply(self, image, hlabel, mask_name, h_pos, **params):
        ...
        assert len(hlabel) == 237 or len(hlabel) == 235, (
            "make sure the rec dataset includes mask params"
        )
        if len(hlabel) == 237:
            if hlabel[1] < 0.0:
                return image
...
if __name__ == "__main__":
    ...
    mask_out = tool.render_mask(
        image, "mask_blue", params
    )  # use single thread to test the time cost
